...
└── source/            # Single source of truth — edit these files
    ...
    ├── skeleton.json  # Project name + activities + steps (no stories)
...
- `source/skeleton.json` — `{ project, activities: [{ id, title, steps: [{ id, title }] }] }`. No stories. `project` is the host name shown in the viewer's title and `<h1>` as `"<project> — User Story Map"`; `/af-init` sets it to the repo directory name. If absent, the viewer falls back to `"af"`.
...
produce `window.USM_DATA = { project, slices, activities }`. The inline SPEC
