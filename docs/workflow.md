...
PM: /af-create-spec <US-####> → Lead writes SPEC content inline on the US (or, for cross-cutting / multi-SPEC work, a SPEC-####.json under source/specs/) → PM confirms → US status: ready (or SPEC status: ready for the standalone path)
            (or /af-create-spec <slice-id> → specs every un-spec'd US in the slice in one session, US by US, skipping any not defined yet)
